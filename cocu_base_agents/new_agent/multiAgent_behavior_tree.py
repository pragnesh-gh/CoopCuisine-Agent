...
class OneShot(py_trees.decorators.Decorator):
    ...
    def update(self):
        # If a previous schedule attempt for this node was rejected by the agent
...
class BehaviorTreeBuilder:
    ...
    def build(state: dict,
        ...
        def pick_board():
            # Minimal + robust chooser that uses outer-scope state/cutting_boards/counters
            if not cutting_boards:
                return None
            owner = getattr(BB, "agent_owner", None) or f"{os.getenv('AGENT_ID', 'A')}-{os.getpid()}"
            # Prefer the board closest to our plate staging spot (diverges naturally per agent)
        ...
        def ensure_plate_staged(seq) -> bool:
            ...
            Additionally, try to RESERVE the chosen spot via Coordinator to avoid
            both agents selecting the same staging tile.
            """
            ...
            if not getattr(BB, "plate_spot_id", None):
                ...
                _add_goto(seq, chosen);    _add_put(seq, chosen)
